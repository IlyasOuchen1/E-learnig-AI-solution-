...
def display_results():
    ...
    filtered_results = {
        ...
        "formatted_objectives": results["formatted_objectives"],
        "difficulty_evaluation": results["difficulty_evaluation"]
    }
    st.download_button(
        label="📥 Télécharger les résultats (JSON)",
        data=json.dumps(filtered_results, ensure_ascii=False, indent=2),
        file_name=f"analyse_objectifs_{st.session_state.session_id[:8]}.json",
        mime="application/json",
        help="Téléchargez les résultats de l'analyse au format JSON"
    )
